export type Product = {
  id: number;
  name: string;
  price: number;
  image: string;
  category: 'biryani' | 'curries' | 'snacks' | 'breads' | 'desserts' | 'beverages';
  description: string;
};

export const products: Product[] = [
  /* --- RAJASTHANI CURRIES (Signature Dishes) --- */
  { id: 1, name: "Dal Baati Churma (Royal Thali)", price: 399, image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT6ukXADndeB3gNj_oa9i38HsXBACxqLoewuQ&s", category: "curries", description: "The heart of Rajasthan. 4 Baatis, Panchmel Dal, and rich Desi Ghee Churma." },

  { id: 3, name: "Ker Sangri (Desert Beans)", price: 249, image: "https://images.squarespace-cdn.com/content/v1/578753d7d482e9c3a909de40/1633003521690-VJA2SHVJY7PEA6IRQ862/201111415_1419030101793610_6241614431259767327_n.jpg?format=1500w", category: "curries", description: "Traditional desert beans and berries cooked with dry fruits and yogurt." },
  { id: 4, name: "Gatte ki Sabzi (Jodhpuri)", price: 199, image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSZNPGxmhVWa6IGzZev1gb1rFabxmpsUZeX-Q&s", category: "curries", description: "Gram flour dumplings in a spicy and tangy yogurt-based curry." },

  { id: 6, name: "Rajasthani Kadhi Pakoda", price: 249, image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTnX0ArQ3RW-EMPG2yhsT4EAnG451NPJ_CU5w&s", category: "curries", description: "Sour yogurt curry tempered with mustard seeds and spicy gram flour fritters." },
  { id: 9, name: "Aloo Pyaaz Paneer (Jaipuri)", price: 329, image: "https://www.indianhealthyrecipes.com/wp-content/uploads/2022/01/paneer-do-pyaza-recipe.webp", category: "curries", description: "Small potatoes and baby onions cooked with paneer in a thick gravy." },

  /* --- RAJASTHANI SNACKS & STARTERS (ID: 11-20) --- */
  { id: 11, name: "Pyaaz ki Kachori (Jodhpuri)", price: 49, image: "https://static.toiimg.com/thumb/59606392.cms?imgsize=319995&width=800&height=800", category: "snacks", description: "Crispy fried pastry stuffed with a spicy onion filling. Famous worldwide." },
  { id: 12, name: "Mawa Kachori", price: 89, image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQAH_iQ--p3RP-PSRti3OiUNi0aUMChUGBESQ&s", category: "snacks", description: "Deep-fried sweet kachori stuffed with khoya and dry fruits, dipped in sugar syrup." },
  { id: 13, name: "Mirchi Bada (Jodhpuri)", price: 39, image: "https://static.toiimg.com/thumb/55836049.cms?width=1200&height=900", category: "snacks", description: "Large green chilies stuffed with spiced potatoes, gram flour batter fried." },
  { id: 14, name: "Bikaneri Bhujia (Fresh)", price: 120, image: "https://instamart-media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,h_600/NI_CATALOG/IMAGES/ciw/2025/12/18/fe8879b4-533d-4f5b-9b23-652cb72c3a13_CEWGDA6CCY_MN_17122025.png", category: "snacks", description: "Crunchy tea-time snack made of moth beans and spices." },
  { id: 15, name: "Shahi Raj Kachori", price: 159, image: "https://thumbs.dreamstime.com/b/shahi-raj-kachori-23333652.jpg", category: "snacks", description: "The King of all kachoris, stuffed with sprouts, dahi, and royal chutneys." },

  { id: 18, name: "Moong Dal Chilla", price: 90, image: "https://ministryofcurry.com/wp-content/uploads/2022/06/Mung-Dal-Chilla_.jpg", category: "snacks", description: "Healthy yellow lentil pancakes stuffed with paneer and herbs." },
  { id: 19, name: "Masala Papand (Roasted)", price: 45, image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRQKgCa1qHJE2OJ8unmBT6D2uork1q_MqVxDQ&s", category: "snacks", description: "Crunchy papad topped with spicy onion, tomato, and lime." },
  { id: 20, name: "Dal Pakwan", price: 149, image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTLd46fL8A3CC3uWSstsDmk6tMTQwR9pPphjA&s", category: "snacks", description: "Crispy fried flatbread served with spicy chana dal." },

  /* --- RAJASTHANI BREADS (ID: 21-30) --- */
  { id: 21, name: "Bajre ki Roti (with Gud & Ghee)", price: 45, image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT54foyO4LtHR2xzqLTTKrFZEg2DxFxLPoEew&s", category: "breads", description: "Pearl millet flatbread, best enjoyed with white butter and jaggery." },

  { id: 24, name: "Batti (Plain - 2pc)", price: 40, image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSqvXBITErxu0Dv9pJ7s4sau3lO8Tr4XhnI_g&s", category: "breads", description: "Traditional baked wheat balls dipped in pure desi ghee." },
  { id: 25, name: "Masala Batti (Stuffed)", price: 49, image: "https://www.awesomecuisine.com/wp-content/uploads/2011/02/Masala-Bati.jpg", category: "breads", description: "Baked battis stuffed with spicy peas and potato masala." },

  { id: 28, name: "Garlic Butter Naan", price: 75, image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRju35j-Dc0_e_pKjWwcyYVRaaBq2VCMqRASg&s", category: "breads", description: "Soft clay oven bread with burnt garlic and fresh butter." },

  { id: 30, name: "Lachha Paratha", price: 65, image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQtzwvbPp5689HfM4SlWDSET42G51Iy6utPOQ&s", category: "breads", description: "Multi-layered flaky bread with a crispy golden finish." },

  /* --- DESERT DESSERTS (ID: 31-40) --- */
  { id: 31, name: "Malai Ghevar (Seasonal)", price: 249, image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS0AJB-TiGJ6K2AXHFAdzGZpwI_IKTAvu1X3w&s", category: "desserts", description: "Jaipur's pride. Honeycomb sweet topped with saffron malai and silver leaf." },
  { id: 32, name: "Gajak (Marwari Special)", price: 120, image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSkBDs5IEXa04POOHg_hbTIncRzTQnvqbp89w&s", category: "desserts", description: "Crunchy winter sweet made with sesame seeds and jaggery." },
  { id: 33, name: "Moong Dal Halwa", price: 189, image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcReSuoOelBTYsrad5ARkuuzDLtzZ77jVo_wEA&s", category: "desserts", description: "Rich, slow-cooked yellow lentil halwa dripping with desi ghee." },


  { id: 38, name: "Churma (Desi Ghee - 1 Plate)", price: 149, image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSDzqiw48mKS692aKPr-tlkldzG5G-TRoku6w&s", category: "desserts", description: "Coarse ground wheat balls mixed with ghee, sugar, and dry fruits." },

  { id: 40, name: "Paneer Ghevar", price: 269, image: "https://lmbsweets.com/wp-content/uploads/2020/07/ghewar.jpg", category: "desserts", description: "Modern twist to the traditional ghevar with a cottage cheese base." },

  /* --- RAJASTHANI BEVERAGES & MORE (ID: 41-50) --- */
  { id: 41, name: "Masala Chhaas (Jaljeera)", price: 49, image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT6sSxE7bOYZbELdcdj80RTNDr5gxRe7Z64MQ&s", category: "beverages", description: "The ultimate thirst quencher. Spicy buttermilk with roasted cumin." },

  { id: 50, name: " (Local Style)", price: 40, image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRIvQa4iZpgnD_KBabMuoxukJYNrqYuSTVIPw&s", category: "beverages", description: "Tangy soda with black salt and secret Marwari spice mix." }
];

// export type Product = { id: number; name: string; price: number; image: string; category: 'biryani' | 'curries' | 'snacks' | 'breads' | 'desserts' | 'beverages'; description: string; };
// export const products: Product[] = [
//   {
//     "id": 1,
//     "name": "Hyderabadi Biryani",
//     "price": 229,
//     "image": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQnSAFpoQta8KtfGYjFwF-dbihuYvYt_SXDzw&s",
//     "category": "biryani",
//     "description": "Freshly prepared Hyderabadi Biryani from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 2,
//     "name": "Lucknowi Biryani",
//     "price": 199,
//     "image": "https://images.unsplash.com/photo-1599487488170-d11ec9c172f0?q=80&w=1200&auto=format&fit=crop",
//     "category": "biryani",
//     "description": "Freshly prepared Lucknowi Biryani from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 3,
//     "name": "Veg Dum Biryani",
//     "price": 199,
//     "image": "https://www.indianhealthyrecipes.com/wp-content/uploads/2013/03/hyderabadi-vegetable-dum-biryani-recipe.jpg",
//     "category": "biryani",
//     "description": "Freshly prepared Veg Dum Biryani from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 4,
//     "name": "Chicken Biryani",
//     "price": 79,
//     "image": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTv-mW5pElUpoE8VmiYxI7L5ox4CnwLKhEkeA&s",
//     "category": "biryani",
//     "description": "Freshly prepared Chicken Biryani from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 5,
//     "name": "Mutton Biryani",
//     "price": 299,
//     "image": "https://images.unsplash.com/photo-1599487488170-d11ec9c172f0?q=80&w=1200&auto=format&fit=crop",
//     "category": "biryani",
//     "description": "Freshly prepared Mutton Biryani from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 6,
//     "name": "Paneer Butter Masala",
//     "price": 299,
//     "image": "data:image/jpeg;base64,",
//     "category": "curries",
//     "description": "Freshly prepared Paneer Butter Masala from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 7,
//     "name": "Dal Makhani",
//     "price": 249,
//     "image": "https://www.cookwithmanali.com/wp-content/uploads/2019/04/Restaurant-Style-Dal-Makhani.jpg",
//     "category": "curries",
//     "description": "Freshly prepared Dal Makhani from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 8,
//     "name": "Chole Masala",
//     "price": 229,
//     "image": "https://www.cookwithmanali.com/wp-content/uploads/2023/04/Punjabi-Chole-Masala-500x500.jpg",
//     "category": "curries",
//     "description": "Freshly prepared Chole Masala from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 9,
//     "name": "Palak Paneer",
//     "price": 229,
//     "image": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT_I8R5TXGoabQURaYGUMiLe7d1GIcdpLdTLQ&s",
//     "category": "curries",
//     "description": "Freshly prepared Palak Paneer from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 10,
//     "name": "Chicken Tikka Masala",
//     "price": 229,
//     "image": "https://www.indianhealthyrecipes.com/wp-content/uploads/2022/06/chicken-tikka-masala.jpg",
//     "category": "curries",
//     "description": "Freshly prepared Chicken Tikka Masala from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 11,
//     "name": "Samosa",
//     "price": 129,
//     "image": "https://static.toiimg.com/photo/61050397.cms",
//     "category": "snacks",
//     "description": "Freshly prepared Samosa from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 12,
//     "name": "Kachori",
//     "price": 99,
//     "image": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQqO0ieyO6huc4rGemgOq5lK5L-66kFU7BwPA&s",
//     "category": "snacks",
//     "description": "Freshly prepared Kachori from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 13,
//     "name": "Pani Puri",
//     "price": 79,
//     "image": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ5hQNVeecPsnSn7Pr-9KibCXlnN8KLj6FzuQ&s",
//     "category": "snacks",
//     "description": "Freshly prepared Pani Puri from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 14,
//     "name": "Bhel Puri",
//     "price": 179,
//     "image": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQoTLg5HeSJalWI5kEVU9vTGL2Hb6oPCmkFmw&s",
//     "category": "snacks",
//     "description": "Freshly prepared Bhel Puri from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 15,
//     "name": "Pav Bhaji",
//     "price": 49,
//     "image": "https://www.cubesnjuliennes.com/wp-content/uploads/2020/07/Instant-Pot-Mumbai-Pav-Bhaji-Recipe.jpg",
//     "category": "snacks",
//     "description": "Freshly prepared Pav Bhaji from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 16,
//     "name": "Butter Naan",
//     "price": 249,
//     "image": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRUfgxpJ2K5LP4y1yVHNbU32LZ7yEqUu0jTYA&s",
//     "category": "breads",
//     "description": "Freshly prepared Butter Naan from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 17,
//     "name": "Garlic Naan",
//     "price": 69,
//     "image": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSxu7EL4I3YHcOi5-VR1UrZZVm8DuDX1ljdGw&s",
//     "category": "breads",
//     "description": "Freshly prepared Garlic Naan from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 18,
//     "name": "Tandoori Roti",
//     "price": 199,
//     "image": "https://www.cookwithmanali.com/wp-content/uploads/2021/07/Tandoori-Roti-300x300.jpg",
//     "category": "breads",
//     "description": "Freshly prepared Tandoori Roti from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 19,
//     "name": "Lachha Paratha",
//     "price": 69,
//     "image": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS8JCQyqHRMsBXMJWqYrKnRJGUxm39MrRTjVQ&s",
//     "category": "breads",
//     "description": "Freshly prepared Lachha Paratha from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 20,
//     "name": "Roomali Roti",
//     "price": 149,
//     "image": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ7VtQoItm2gmPlSMCKaItUOWwKI3Ii1jqLBQ&s",
//     "category": "breads",
//     "description": "Freshly prepared Roomali Roti from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 21,
//     "name": "Gulab Jamun",
//     "price": 49,
//     "image": "https://www.indianhealthyrecipes.com/wp-content/uploads/2021/11/gulab-jamun.webp",
//     "category": "desserts",
//     "description": "Freshly prepared Gulab Jamun from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 22,
//     "name": "Rasmalai",
//     "price": 129,
//     "image": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTVrggbTNDvmTQKGR5l5mTnPeMvLOlDxMHFMg&s",
//     "category": "desserts",
//     "description": "Freshly prepared Rasmalai from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 23,
//     "name": "Rasgulla",
//     "price": 299,
//     "image": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTPZw5edwhwaxvQAcTikUHSAt0UAgr7A0F6VA&s",
//     "category": "desserts",
//     "description": "Freshly prepared Rasgulla from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 24,
//     "name": "Kheer",
//     "price": 299,
//     "image": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT43RrlU0L08XfvFesK-XXzAOsYPcriUf3GEw&s",
//     "category": "desserts",
//     "description": "Freshly prepared Kheer from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 25,
//     "name": "Jalebi",
//     "price": 299,
//     "image": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSKSnRxn3xZnd7W_9mkXNmBippHtW1u7iu4ow&s",
//     "category": "desserts",
//     "description": "Freshly prepared Jalebi from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 26,
//     "name": "Masala Chai",
//     "price": 79,
//     "image": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR8LMM6j0uSjpwGASdoFVtMLW_iojIyFp6ZfQ&s",
//     "category": "beverages",
//     "description": "Freshly prepared Masala Chai from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 27,
//     "name": "Sweet Lassi",
//     "price": 49,
//     "image": "https://www.sharmispassions.com/wp-content/uploads/2023/08/sweet-lassi5.jpg",
//     "category": "beverages",
//     "description": "Freshly prepared Sweet Lassi from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 28,
//     "name": "Salted Lassi",
//     "price": 79,
//     "image": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSDJl5giiE26PIj2AiDVRiVzwuqVuXVJX7MHg&s",
//     "category": "beverages",
//     "description": "Freshly prepared Salted Lassi from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 29,
//     "name": "Badam Milk",
//     "price": 249,
//     "image": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQGo8jttJx_y8Jl1MzmvvHoKLK8q9S1-ViPYQ&s",
//     "category": "beverages",
//     "description": "Freshly prepared Badam Milk from Aditi Restaurant kitchen."
//   },
//   {
//     "id": 30,
//     "name": "Cold Coffee",
//     "price": 79,
//     "image": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR7HN8AiGNBnRRMXEzB8ONaK4QJoAW0zhJ49A&s",
//     "category": "beverages",
//     "description": "Freshly prepared Cold Coffee from Aditi Restaurant kitchen."
//   }
// ];
